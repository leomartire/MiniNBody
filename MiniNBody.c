...
int main(int argc, char *argv[]){
  ...
  strcpy(timeUnit, "seconds");
	for(i=0; i<MAX_NBODY; i++){ g_body_array[i].index=-1; strcpy(g_body_array[i].name, ""); g_body_array[i].mass=-1;}
	g_outfile_fp=(FILE *) NULL;
	strcpy(g_technique_array[g_technique_number].name, "E1"); g_technique_array[g_technique_number++].func=tech_euler_1;
	strcpy(g_technique_array[g_technique_number].name, "E1a"); g_technique_array[g_technique_number++].func=tech_euler_1a;
  strcpy(g_technique_array[g_technique_number].name, "E2"); g_technique_array[g_technique_number++].func=tech_euler_2;
	strcpy(g_technique_array[g_technique_number].name, "RK4"); g_technique_array[g_technique_number++].func=tech_rk4;
  /*---------------------------------------------------------*/
	...
	for(i=1; i<argc; i++){
		if(g_verbose_flag>0){ printf(" arg %d is ..%s.. \n", i, argv[i]);}
		/* read input file name */
		if(strncmp(argv[i], "input=", 6)==0){
			if(strlen(argv[i])>=NBUF){ fprintf(stderr, "inputfile name ..%s.. is too long, max %d chars. \n", argv[i], (int) strlen(argv[i])); exit(1);}
			if(sscanf(argv[i]+6, "%s", inputfile_name)!=1){ fprintf(stderr, "can't read inputfile_name from ..%s.. \n", argv[i]+6); exit(1);}
			if(g_verbose_flag>0){ printf(" read input file name ..%s.. \n", inputfile_name);}
		}
		if(strcmp(argv[i], "verbose")==0){g_verbose_flag=1; printf(" set verbose level to ..%d.. \n", g_verbose_flag);}
		if(strncmp(argv[i], "verbose=", 8)==0){
			if(sscanf(argv[i]+8, "%d", &g_verbose_flag)!=1){ fprintf(stderr, "can't read verbosity level from ..%s.. \n", argv[i]+8); exit(1);}
			if(g_verbose_flag>0){printf(" set verbose level to ..%d.. \n", g_verbose_flag);}
		}
	}
	if(inputfile_name[0]=='\0'){ fprintf(stderr, "no input= argument provided ?! \n"); exit(1);} // Check to make sure we read all required arguments.
  printf("\n> Starting input file reading.\n");
	if(read_input(inputfile_name)!=0){ fprintf(stderr, "read_input fails \n");exit(1);} /* read all input information */
	if(g_recenter_flag==1){if(recenter_velocities()!=0){ fprintf(stderr, "recenter_velocities returns with error \n"); exit(1);}} // If desired, add a constant velocity to all bodies so that the center of mass of the system is motionless.
  /*---------------------------------------------------------*/
  ...
  double energyClock=clock(); // check time before
	...
	if(g_verbose_flag>0){printf("> Initial KE : %12.5e.\n", initial_ke);}
	initial_gpe=calc_total_gpe();
	if(g_verbose_flag>0){printf("> Initial GPE : %12.5e.\n", initial_gpe);}
	initial_tot_e=initial_ke+initial_gpe;
	if(g_verbose_flag>0){printf("> Initial E   : %12.5e.\n", initial_tot_e);}
	if(calc_total_angmom(&initial_ang_mom)!=0){fprintf(stderr, "calc_total_angmom fails for initial \n");exit(1);}
	if(g_verbose_flag>0){
  ...
  if(print_positions(g_outfile_fp, currentSimulationTime)!=0){fprintf(stderr, "Iteration %16d, time %9.4e [T].\n", 0, currentSimulationTime); printError("main", "Function print_positions failed."); exit(1);}
  /*---------------------------------------------------------*/
  ...
  if(g_verbose_flag>0){remainingTimePrintModulo=20000; previousInfoClock=clock();} // every how many iterations print informations about remaining time and dummy value for first previousRemainingRealTime
  printf("\n> Entering main integration loop.\n");
	while(currentSimulationTime<=g_duration){
		/* Call the integration -*/
    /* function.            -*/
		if(g_verbose_flag>1){printf(" >> About to enter the integration function (t=%9.4e).\n",currentSimulationTime);}
    ...
    if(g_usePECE_flag==0 && g_usePEC_flag==0){ // PECE and PEC are desactivated : just use RK4 all the way.
      ...
      if(retval!=0){printError("main", "Integration function failed."); return(1);}
      ...
      if(n<=3){ // For the first 4 steps of PECE, call RK4.
        ...
        if(n==0){stateToDerivativeState(g_body_array, fp3);}
        if(n==1){stateToDerivativeState(g_body_array, fp2);}
        if(n==2){stateToDerivativeState(g_body_array, fp1);}
        if(n==3){stateToDerivativeState(g_body_array, fp0);}
      }
    ...
    /* output file.       ---*/
		...
		if(printing_clock>=g_print_interval){
			...
			if(print_positions(g_outfile_fp, currentSimulationTime)!=0){fprintf(stderr, "Iteration %16d, time %9.4e [T].\n", n, currentSimulationTime); printError("main", "Function print_positions failed."); exit(1);}
			printing_clock=0.0;
		}
    /*-----------------------*/
    /* Print information ----*/
    ...
    if(g_verbose_flag>0){
      if(n==100 || n==1000 || n==2000 || n%remainingTimePrintModulo==0){ // after first 100, first 1000, first 2000 and then every 20000 integrations, estimate the remaining time
        ...
        if((n>2000) && ((clock()-previousInfoClock)<g_minimumRemTimePrintInt*CLOCKS_PER_SEC)){remainingTimePrintModulo*=2;} // if informations prints too fast, raise interval in terms of iterations
        if(remainingRealTime<60){strcpy(timeUnit, "seconds");} if(remainingRealTime>=60 && remainingRealTime<3600){strcpy(timeUnit, "minutes"); remainingRealTime/=60;} if(remainingRealTime>=3600){strcpy(timeUnit, "hours"); remainingRealTime/=3600;} printf("> Iteration %16d, time %9.4e [T], estimated remaining time : %6.2f %s.\n", n, currentSimulationTime, remainingRealTime, timeUnit);
  ...
  energyClock=clock(); // check time before
	...
	if(g_verbose_flag>0){printf("> Final KE  : %12.5e.\n", final_ke);}
	final_gpe=calc_total_gpe();
	if(g_verbose_flag>0){printf("> Final GPE : %12.5e.\n", final_gpe);}
	final_tot_e=final_ke+final_gpe;
	if(g_verbose_flag>0){printf("> Final E   : %12.5e.\n", final_tot_e);}
	if(calc_total_angmom(&final_ang_mom)!=0){fprintf(stderr, "calc_total_angmom fails for final \n");exit(1);}
	if(g_verbose_flag>0){
  ...
  /*---------------------------------------------------------*/
	...
	/* Prepare to exit. ---------------------------------------*/
	if(g_verbose_flag>0){
    if(mainLoopTimeTaken<60){strcpy(timeUnit, "seconds");} if(mainLoopTimeTaken>=60 && mainLoopTimeTaken<3600){strcpy(timeUnit, "minutes"); mainLoopTimeTaken/=60;} if(mainLoopTimeTaken>=3600){strcpy(timeUnit, "hours"); mainLoopTimeTaken/=3600;}
		printf("\n> All done.\n");
...
static int tech_rk4(double suggested_timestep, double *actual_timestep){
  ...
  sixth_step=timestep/6.0;
	...
	if(calc_grav_forces(step2_array, a1)!=0){printError("tech_rk4", "Function calc_grav_forces fails in step a1."); return(1);}
  #pragma omp for
	for(i=0; i<g_body_number; i++){
		for(c=0; c<3; c++){
  ...
  if(calc_grav_forces(step2_array, a2)!=0){printError("tech_rk4", "Function calc_grav_forces fails in step a2."); return(1);}
  #pragma omp for
	for(i=0; i<g_body_number; i++){
		for(c=0; c<3; c++){
	...
	if(calc_grav_forces(step3_array, a3)!=0){printError("tech_rk4", "Function calc_grav_forces fails in step a3."); return(1);}
	#pragma omp for
	for(i=0; i<g_body_number; i++){
		for(c=0; c<3; c++){
	...
	if(calc_grav_forces(step4_array, a4)!=0){printError("tech_rk4", "Function calc_grav_forces fails in step a4."); return(1);}
	#pragma omp for
	for(i=0; i<g_body_number; i++){
		for(c=0; c<3; c++){
...
static int calc_grav_forces(BODY *body_array, VECTOR *forces){
  ...
  if(g_galactic_flag==1){add_galactic_effect(forces);} // if the effect of a galaxy is wanted, add it to the current forces
	return(0);
}
...
static int recenter_velocities(void){
  ...
  // @param none
	...
	total_mass=0.0;
	for(c=0; c<3; c++){momentum.val[c]=0.0;}
	for(i=0; i<g_body_number; i++){total_mass+=g_body_array[i].mass; for(c=0; c<3; c++){momentum.val[c]+=g_body_array[i].mass*g_body_array[i].vel[c];}}
	if(total_mass==0.0){return(0);} // If the total mass is zero, just return now (why would that be ever the case anyways ?).
	for(c=0; c<3; c++){com_velocity.val[c]=momentum.val[c] / total_mass;} // Compute the velocity of the center of mass.
  #pragma omp for
	for(i=0; i<g_body_number; i++){for(c=0; c<3; c++){g_body_array[i].vel[c] -= com_velocity.val[c];}} // Subtract this velocity from each body.
	return(0);
}
...
static double calc_total_ke(void){
  ...
  // @return the total KE
	...
	total_ke=0;
	for(i=0; i<g_body_number; i++){
		vSquared=g_body_array[i].vel[0]*g_body_array[i].vel[0] 
		         +g_body_array[i].vel[1]*g_body_array[i].vel[1] 
		         +g_body_array[i].vel[2]*g_body_array[i].vel[2]; 
		ke=0.5*g_body_array[i].mass*vSquared;
		total_ke+=ke;
		if(g_verbose_flag>1){printf(">> [calc_total_ke] : Body %5d has v**2=%9.4e and m=%9.4e, thus %9.4e of KE (total updated to %9.4e).\n", i, vSquared, g_body_array[i].mass, ke, total_ke);}
	}
	return(total_ke);
}
  ...
  // @return the total GPE
	...
	for(i=0; i<g_body_number-1; i++){
		for(j=i+1; j<g_body_number; j++){
			...
			d=sqrt(d_squared);
			if(d<=0){fprintf(stderr, "Distance %9.4le (between bodies %d and %d).\n", d, i, j); printError("calc_total_gpe", "Distance is invalid."); exit(1);}
			gpe=0.0-((g_G*g_body_array[i].mass*g_body_array[j].mass) / d);
			total_gpe+=gpe;
			if(g_verbose_flag>1){printf(">> [calc_total_gpe] Bodies %5d and %5d have %9.4e of GPE (total GPE updated to %9.4e).\n", i, j, gpe, total_gpe);}
		}
	}
	return(total_gpe);
}
...
static int calc_total_angmom(VECTOR *result){
  ...
  // *result total angular momentum of system
	...
	for(i=0; i<g_body_number; i++){
		...
		for(c=0; c<3; c++){total_ang_mom.val[c]+=body_ang_mom.val[c];}
	}
	for(c=0; c<3; c++){result->val[c]=total_ang_mom.val[c];} // Copy the total angular momentum to output argument.
	return(0);
}
...
static int print_positions(FILE *outfile_fp, double currentSimulationTime){
  ...
  // @param currentSimulationTime current simulation time
	int i;
	if(outfile_fp==NULL){printError("print_positions", "File was given a NULL pointer."); return(1);}
...
static int read_input(char *filename){
  ...
  // @param *filename name of ASCII text file with input information
	...
	if((fp=fopen(filename, "r"))==NULL){fprintf(stderr, "File : \"%s\".\n", filename); printError("read_input", "Can't open file."); return(1);}
  /*---------------------------------------------------------*/

  /* Start to read file. ------------------------------------*/
	rewind(fp);
	while (fgets(line, LINELEN, fp)!=NULL){
		if(g_verbose_flag>1){printf(">> Next line in input file is :\n%s", line);}if(line[0]==COMMENT_CHAR){if(g_verbose_flag>1){printf(">> Skipping comment line.\n");}continue;}if(line[0]=='\n'){if(g_verbose_flag>1){printf(">> Skipping empty line.\n");}continue;}
    ...
    */
      ...
      if(g_integration_func==NULL){
        ...
        return(1);
			...
			if(g_verbose_flag>0){
        if(g_usePECE_flag==1){if(g_usePEC_flag==1){printf(">> Technique PEC (initialisations with %s) will be used.\n", technique);}else{printf(">> Technique PECE (initialisations with %s) will be used.\n", technique);}}else{printf(">> Technique %s will be used.\n", technique);}
			}
		}
    ...
    */
      ...
      if(strlen(this_name)>=NAMELEN){fprintf(stderr, "ERROR [read_input] : Body with name \"%s\" has name longer than %d chars.\n", this_name, NAMELEN); return(1);}
      if((this_index<0) || (this_index>=g_body_number)){fprintf(stderr, "ERROR [read_input] : Body with name \"%s\" has invalid index %d.\n", this_name, this_index); return(1);}
      if(this_mass<=0){fprintf(stderr, "ERROR [read_input] : Body with name \"%s\" has invalid mass %lf.\n", this_name, this_mass); return(1);}
      // Now try to set entries in the appropriate g_body_array[].
      if(g_body_array[this_index].index!=-1){fprintf(stderr, "ERROR [read_input] : Body with name \"%s\" has repeated index %d.\n", this_name, this_index); return(1);}
      else {
        ...
        g_body_array[this_index].pos[0]=this_px; g_body_array[this_index].pos[1]=this_py; g_body_array[this_index].pos[2]=this_pz;
        g_body_array[this_index].vel[0]=this_vx; g_body_array[this_index].vel[1]=this_vy; g_body_array[this_index].vel[2]=this_vz;
      }
      if(g_verbose_flag>1){
    ...
    */
    double mb, ab, md, ad, hd, vh, ah;
		if(strncmp(line, "galParam", 8)==0){
			if(sscanf(line+8, " %lf %lf %lf %lf %lf %lf %lf", 
              &mb, &ab, &md, &ad, &hd, &vh, &ah)!=7){
  ...
  // Check number of bodies in the simulation.
	if((g_body_number<1) || (g_body_number>=MAX_NBODY)){fprintf(stderr, "ERROR [read_input] : nbody %d is<1 or>MAX_NBODY (%d).\n", g_body_number, MAX_NBODY); return(1);}
  // Check duration of the simulation.
	if(g_duration<0){fprintf(stderr, "ERROR [read_input] : g_duration %lf is<0 (probably not correctly set).\n", g_duration); return(1);}
  // Check timestep of the simulation.
  if(g_timestep<=0){fprintf(stderr, "ERROR [read_input] : g_timestep %lf is<0 (probably not correctly set).\n", g_timestep); return(1);}
  // Check printing interval.
	if(g_print_interval<=0){fprintf(stderr, "ERROR [read_input] : g_print_interval %lf is<0 (probably not correctly set).\n", g_print_interval); return(1);}
  // Check recenter flag.
	if(g_recenter_flag<0){fprintf(stderr, "ERROR [read_input] : g_recenter_flag not set.\n"); return(1);}
  // Check technique.
	if(g_integration_func==NULL){fprintf(stderr, "ERROR [read_input] : No technique provided in %s.\n", filename); return(1);}
  // Check output file.
  if(g_outfile_fp==NULL){fprintf(stderr, "ERROR [read_input] : No valid outfile supplied.\n"); return(1);}
  // Check softening.
  if(g_softening==-1){fprintf(stderr, "ERROR [read_input] : g_softening %lf is<0 (probably not correctly set).\n", g_softening); return(1);}
  // Check gravitational constant.
  if(g_G==-1){fprintf(stderr, "ERROR [read_input] : g_G %lf is<0 (probably not correctly set).\n", g_G); return(1);}
  // Check galactic flag.
	if(g_galactic_flag<0){fprintf(stderr, "ERROR [read_input] : g_galactic_flag not set.\n"); return(1);}
  ...
  if(g_galactic_flag==1 && (g_mb<=0 || g_ab<=0 || g_md<=0 || g_ad<=0 || g_hd<=0 || g_vh<=0 || g_ah<=0)){fprintf(stderr, "ERROR [read_input] : Galaxy parameters must all be>0.\n"); return(1);}
  /*---------------------------------------------------------*/
